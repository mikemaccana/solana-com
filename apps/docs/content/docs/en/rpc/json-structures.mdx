---
title: Solana RPC JSON Structures Reference
hideTableOfContents: false
h1: Solana RPC JSON Structures Reference
---
...
Transactions included in the block. Present when `transactionDetails` is
`"full"` or `"accounts"`.
...
Transaction metadata, or `null` if metadata is unavailable.
...
schema documented in [Account Objects](#account-objects). The account object's
`data` field depends on the requested account encoding.
...
Program log output captured during simulation. Current Agave always serializes
this field as an array; it may be empty.

### !! replacementBlockhash

!type object | null

Replacement blockhash Agave used when `replaceRecentBlockhash` was enabled.
...
binary encodings, Agave returns a base-58/base-64 string or a
`[string, encoding]` tuple.
...
| `jsonParsed` | `UiParsedMessage` | Parsed or partially decoded instructions | `accountKeys` are objects, and current Agave includes lookup-table accounts in parsed versioned responses.                                                             |
...
transaction is versioned and `maxSupportedTransactionVersion` is set. Current
Agave emits this field for v0 messages even when the array is empty. Uses the
...
Transaction-level instruction stack height. Current Agave emits `1` for
...
List of account information used by the transaction. In current Agave full
`jsonParsed` responses for versioned transactions, this includes loaded
...
Source of the account metadata. In current Agave transaction/account encoders,
this field is populated as `transaction` for static message accounts and
`lookupTable` for loaded lookup-table accounts.
...
instruction was partially decoded because Agave did not have a parser for the
program.
...
decoded because Agave did not have a parser for the program.
...
When a block is requested with `transactionDetails: "accounts"`, Agave returns
transaction account metadata in this structure:
...
Account data size in bytes. Current Agave populates this field for encoded
account responses.
...
available, Agave falls back to `[data, "base64"]`.
...
Source of the account metadata. Current Agave sets this to `transaction` for
static message accounts and `lookupTable` for loaded lookup-table accounts in
...
The table below highlights fields whose presence changes across response modes.
Fields not listed here serialize the same way whenever metadata is available.
...
Transaction error. `null` indicates success. Unit variants serialize as strings;
tuple and struct variants serialize as tagged objects. `InstructionError`
serializes as `{"InstructionError": [instructionIndex, instructionError]}`.
...
non-`jsonParsed` transaction metadata responses. Current Agave always serializes
this field as an object, using empty arrays when no addresses were loaded.
Omitted for `jsonParsed` and `accounts` responses. Uses the structure documented
in [Loaded Addresses](#loaded-addresses).
...
Deprecated failure payload. Uses the same serialization as `err`.
...
```jsonc !
// !hover(1:8) transactionError
{
  // !hover(1:6) transactionError.InstructionError
  "InstructionError": [
    5,
    {
      "Custom": 6042
...
Unit variants serialize as a string, such as `"BlockhashNotFound"`.
...
Instruction error tuple containing the outer instruction index and the
serialized instruction error.
...
Return data payload and encoding. Current Agave returns base-64 data with
`"base64"` encoding.
